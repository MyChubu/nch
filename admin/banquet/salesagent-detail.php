...
$sql = "SELECT * FROM `banquet_agents` WHERE `agent_id` = :agent_id";
$stmt = $dbh->prepare($sql);
...
$count = $stmt->rowCount();
if($count > 0) {
  $agent_name= $result[0]['agent_group'];
  $agent_short= $result[0]['agent_group_short'];
}else{
  header("Location: salesagents.php");
  exit;
}
      ...
      <?php if(sizeof($sales) > 0): ?>
        ...
        <?php foreach($sales as $row):?>
          <?php
            ...
            if($agent_id == 2999 ){
              $sales_office = cleanLanternName($sales_office);
            }
          ?>
              <tr>
                <td><?=$sales_office ?></td>
