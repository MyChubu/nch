...
if($cat_id == "" || $cat_id > 6) {
  header("Location: salesyear.php?nendo=$nendo");
  exit;
}
...
$categories = array();
$sql = "SELECT * FROM `banquet_categories` WHERE `banquet_category_id` < 9 ORDER BY `banquet_category_id`";
$stmt = $dbh->prepare($sql);
$stmt->execute();
...
if($count > 0) {
  foreach($results as $result) {
    $categories[$result['banquet_category_id']] = $result['banquet_category_name'];
    if($result['banquet_category_id'] == $cat_id) {
...
$sql = "SELECT 
  ...
  AND `sales_category_id` = :cat_id
  GROUP BY `ym`,`reservation_id`,`sales_category_id`,`reservation_date`
...
if($count > 0) {
  foreach($results as $result) {
    ...
    $sales[] = array(
      ...
      'pic' => cleanLanternName($result['pic']),
    ...
    <form  enctype="multipart/form-data" id="schedate_change">
      ...
      <select name="cat" id="cat">
        ...
        <?php foreach($categories as $key => $value): ?>
          <option value="<?=$key ?>" <?=($cat_id == $key)?"selected":"" ?>><?=$value ?></option>
        <?php endforeach; ?>
      </select>
      <button type="submit">変更</button>
    </form>
    <div id="controller_year">
      <div id="before_year"><a href="?nendo=<?= $before_nendo ?>&amp;cat=<?=$cat_id ?>"><i class="fa-solid fa-arrow-left"></i>前年度</a></div>
      <div id="this_year"><a href="?nendo=<?= $this_nendo ?>&amp;cat=<?=$cat_id ?>">今年度</a></div>
      <div id="after_year"><a href="?nendo=<?= $after_nendo ?>&amp;cat=<?=$cat_id ?>">翌年度<i class="fa-solid fa-arrow-right"></i></a></div>
