...
`ranked_reservations` as (
  select `pre_daily_subtotal3`.`sche_id` AS `sche_id`,
...
`pre_daily_subtotal3`.`ex-ts` AS `ex-ts`,
...
row_number() OVER (PARTITION BY `pre_daily_subtotal3`.`ym`,
`pre_daily_subtotal3`.`date`,
`pre_daily_subtotal3`.`room_id` ORDER BY `pre_daily_subtotal3`.`gross` desc )  AS `rn` from `pre_daily_subtotal3`
) 
select 
